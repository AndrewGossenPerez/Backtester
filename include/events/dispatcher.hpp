...
// Used to store events in a ring-buffer for later dispatching. Events are dispatched in this order : 
    ...
    // SignalHandler - ( MarketEvent -> SignalEvent)
...
#include "pipeline/signaler_handler.hpp"
...
class Dispatcher{
    ...
    Dispatcher(Signaller& signaller,trd::MarketState& marketState,Portfolio& portfolio,trd::Result& result) : 
    m_handlerSignal(signaller,*this),
    ...
    void on(const events::FillEvent& ev) { 
        m_handlerPort.on(ev); m_handlerReport.on(ev); m_handlerSignal.on(ev);m_handlerStop.on(ev);
    } 
